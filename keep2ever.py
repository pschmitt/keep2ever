...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Export Google Keep to Evernote")

    parser.add_argument("ImportFile", type=str, help="Google takeout export file(.zip)")
    parser.add_argument("ExportFile", help="Output file", nargs="?", default="")
...
def evernote_resources(json_note, zip_file):
    ...
    for res in json_note["attachments"]:
        ...
        try:
            res_data = zip_file.read(res_path.replace("jpeg", "jpg"))
        ...
        with ZipFile(impmort_file, "r") as zip_file:
            for file in zip_file.namelist():
                if re.match(json_files_re, file):
                    ...
                    notebook_file.write(NOTE_TEMPLATE.format(**template_values))
